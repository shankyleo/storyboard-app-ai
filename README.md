...
# Add DATABASE_URL (Neon) and optionally OPENAI_API_KEY for live beat generation
...
4. **Panels** — pick OpenAI or Gemini (when keys are set), then generate storyboard visuals
...
| `REPLICATE_API_TOKEN` | No | Panel images when OpenAI and Gemini keys are unset; falls back to placeholders |
